...
ExcaLib v1 (`frc.excalib`) was **deleted** once the migration was proven (build green, sim clean, no `frc.excalib` references in `src`). The two `Robot`-loop leftovers (`PeriodicScheduler.run()`, `TalonFXMotor.refreshAll()`) went with it — nothing registered on either. **Monologue was also retired** — `implements Logged`, `@Log.NT`/`@NT`, and `Monologue.setupMonologue/updateAll` are all gone, and the vendordep was removed from `build.gradle`. DogLog is now the sole telemetry system (WPILib DataLog + NT + Phoenix SignalLogger). The migration and all legacy cleanup are complete.
