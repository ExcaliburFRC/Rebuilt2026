...
/**
 * Periodic device-fault scanner. Every registered TalonFX's key faults surface as dashboard
 * {@link Alert}s and DogLog faults; RIO CAN-bus health is logged and alerted on. Rate-limited
 * internally to 1 Hz — call {@link #poll()} every loop from {@code robotPeriodic}.
 *
 * <p>{@link TalonFX}-based motors register themselves automatically, so every motor built
 * through the library is covered with no per-subsystem code. This replaces the old
 * {@code CANHealthMonitor} (which only watched the bus-off count via SmartDashboard).
 *
...
public final class FaultReporter {
    ...
    private static final class MonitoredDevice {
        ...
        MonitoredDevice(String name, TalonFX talon) {
            ...
            // Give the fault signals an explicit low rate so the bus-utilization optimize pass
            // (PhoenixSignalHub.optimizeAll) keeps them alive instead of disabling them.
            BaseStatusSignal.setUpdateFrequencyForAll(4.0,
                    hardwareFault, deviceTempFault, bootDuringEnable, undervoltage);
        }
    ...
    /** Registers a TalonFX for fault scanning. The library motor wrapper does this automatically. */
    public static void register(String name, TalonFX talon) {
        DEVICES.add(new MonitoredDevice(name, talon));
    }

    /** Scans all registered devices and CAN health at most once per second. Call every loop. */
    ...
    /** RIO CAN bus health logging + utilization alert (old {@code CANHealthMonitor} equivalent). */
