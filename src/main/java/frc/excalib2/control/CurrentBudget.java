 ...
 *   <li><b>Supply limit</b> — protects the breaker and the battery. When the supply current
 *       exceeds {@code supplyLimit} for {@code supplyLowerTime} seconds, the limit drops to
 *       {@code supplyLowerLimit} (breaker-trip protection / brownout mitigation).</li>
 *   <li><b>Torque-current peaks</b> — hard output clamp for {@link ControlMode#TORQUE_CURRENT_FOC}
 *       requests (which bypass the stator limit by design in Phoenix 6).</li>
 * </ul>
 *
 * <p>Robot code should keep every mechanism's budget in one named table so the whole
 * robot's current allocation is reviewable at a glance.
 */
