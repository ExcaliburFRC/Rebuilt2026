package frc.excalib2.swerve.vision;
 ...
 * dependency on robot code or the full LimelightHelpers).
 *
 * <p>Contract: call {@link #setRobotOrientation} every loop with the best current heading
 * (MT2 solves translation given orientation), then consume {@link #getEstimate()} and feed
 * it to the drivetrain pose estimator with {@link #stdDevs} scaling.
 */
public class LimelightMegaTag2 {
    ...
    /**
     * Publishes the robot heading MegaTag2 needs. For a turret-mounted camera, pass the
    ...
    /**
     * Vision std-devs scaled by distance and tag count: trust falls off with square of
     * distance, improves with multiple tags. MT2 heading is not trusted at all (the
