...
/**
 * A live-tunable number published under {@code /Tuning/<key>} — edit from AdvantageScope /
 * Elastic while the robot runs. When tuning mode is off (competition default) the compiled
 * default is always returned and nothing is published.
 *
 * <p>{@link #ifChanged} runs a callback when any of a group of tunables changes — the hook
 * for re-applying gains to a device.
 *
 * <p>Pattern credit: 6328 {@code LoggedTunableNumber}, re-expressed over plain
 * NetworkTables (no AdvantageKit).
 */
